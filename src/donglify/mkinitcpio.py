...
def clean_mkinitcpio_config(current_install_name):
    cmd = "rm /etc/mkinitcpio.conf"
    execute(cmd, desc=f'remove /etc/mkinitcpio.conf')
    
    cmd = "rm /etc/initcpio/install/sd-sulogin"
    execute(cmd, desc=f'remove /etc/initcpio/install/sd-sulogin')

    cmd = "rm /etc/shadow.initramfs"
    execute(cmd, desc=f'remove /etc/shadow.initramfs')

    
def kernel_config_current_sys(current_install_name: str):
    setup_mkinitcpio_config(current_install_name)
